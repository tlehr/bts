...
function update_official_tables(officials_host) {
  ...
  const planned_rows = curt.umpires
    .filter(o => o.is_planed_as_umpire || o.is_planed_as_service_judge)
    .sort((a, b) => (a.name || '').localeCompare(b.name || ''));

  if (planned_rows.length > 0) {
    uiu.el(officials_div, 'h3', 'edit', ci18n('Assigned to matches in preparation:'));
    render_officials_table(officials_div, {
      rows: planned_rows,
      table_id: 'planned_assignment',
      min_height_px: 120
    });
  }
